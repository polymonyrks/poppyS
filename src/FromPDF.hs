...
nounJTag = CJTag {jTag = "名詞", jTag1 = "名詞名詞", jTagIsNP = True}
...
foldAdvAdjJP sexp0 = sexp1
  where
    foldVPNP = foldSuccTags (verbJTag, 2, f)
      where
        verbJTag = CJTag "形容詞" "副詞形容詞" False
        f xs = isHeadNP && isLastVP
          where
