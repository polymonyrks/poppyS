...
### モード切替
Upキーもしくはvキーで読書モードの切り替えができます。デフォルトではHintモードです。これは、先述のとおりに重要語が黄色でマーキングされるモードです。  
...
UpキーもしくはDownキーでHint, Gramatica, Primitiveを切り替えます。
vキーでHintモードとVanillaモードの切り替えをします。普段はこちらを使う形になります。  

# 戦略とトレーニング
着色は戦略的に行わないと逆に読みにくくなります。ある程度ノウハウを持っており、このHPでも度々ご紹介していますが、  
折を見てここでまとめるつもりです(電子書籍の形でまとめることにしました)。  
